...
const PERMS=[
  ...
  ['editar_documentos','Gestionar fotos y documentos'],['gestionar_informes','Informes a familias'],
...
function rolePreset(r){const all=Object.fromEntries(PERMS.map(p=>[p[0],true]));const none=Object.fromEntries(PERMS.map(p=>[p[0],false]));const ON=(...k)=>{const o={...none};k.forEach(x=>o[x]=true);return o;};
  switch(r){
    ...
    case 'profesor':return ON('ver_dashboard','ver_estadisticas','ver_alumnos','gestionar_informes');
...
function freshDB(){return {v:DB_VERSION,
  ...
  mensajes:[],
  savedViews:[],informePlantillas:DEFAULT_INFORME_PLANTILLAS(),
...
function loadDB(){
  // Si hay backend, NO usar copia local (puede ser de otro colegio/usuario). El bridge cargará la nube.
  if(window.AuloraBackend&&window.AuloraBackend.enabled){try{localStorage.removeItem('aulora_db');}catch(e){}DB=freshDB();return;}
...
function inscribirAlumnoCurso(sid,cursoNombre,ciclo){if(!(perm('ver_inscripciones'))){toast('Sin permiso.','danger');return;}ciclo=ciclo||inscCiclo;const s=DB.students.find(x=>x.id===sid);const c=courseByName(cursoNombre);if(!s||!c)return;
...
function confirmarVacante(sid,ciclo){if(!(perm('ver_inscripciones'))){toast('Sin permiso.','danger');return;}ciclo=ciclo||inscCiclo;const s=DB.students.find(x=>x.id===sid);const i=getIns(s,ciclo);if(!i)return;const c=courseByName(i.curso);
...
function toggleAct(sid,aid){if(!(perm('ver_actividades')||perm('registrar_actividades'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const a=getAct(aid);const e=(s.actividades||[]).find(x=>x.id===aid);
...
function viewAlumnoDetalle(){const s=DB.students.find(x=>x.id===openStudentId);if(!s)return'';const ins=getIns(s,cfg().ciclo);
  ...
  const tabs=[['info','Datos'],['academico','Académico'],['salud','Salud y retiro'],['convivencia','Convivencia'],['pagos','Cuotas'],['documentos','Documentos'],['observaciones','Observaciones'],['informes','Informes']];
  return `<div class="view"><button class="btn btn-ghost btn-sm" style="margin-bottom:14px" onclick="go('alumnos')">← Volver a alumnos</button>
   ...
   ${detailTab==='info'?tabInfo(s):detailTab==='academico'?tabAcademico(s):detailTab==='salud'?tabSalud(s):detailTab==='convivencia'?tabConvivencia(s):detailTab==='pagos'?tabPagos(s):detailTab==='documentos'?tabDocumentos(s):detailTab==='observaciones'?tabObservaciones(s):tabInformes(s)}</div>`;}
...
function viewConfig(){if(!perm('editar_config'))return noPerm();const c=cfg();
  ...
  </div>${panelFormAlumno()}${panelInformes()}</div>`;}
...
/* ====================== PORTAL FAMILIA ====================== */
let famFileData=null,tmpFoto=null,tmpMaterias=[],tmpCourseId=null,tmpComp=null;
function famUpload(){const f=$('#famFile').files[0];if(!f)return;const isPdf=/pdf$/i.test(f.type)||/\.pdf$/i.test(f.name);const done=d=>{famFileData={file:d,nombre:f.name,tipo:isPdf?'pdf':'image'};$('#famPreview').innerHTML=`<div class="chip">📎 ${esc(f.name)} <button onclick="famFileData=null;$('#famPreview').innerHTML='';$('#famFile').value=''" style="color:var(--danger);font-weight:700;margin-left:4px">×</button></div>`;};if(isPdf){const r=new FileReader();r.onload=e=>done(e.target.result);r.readAsDataURL(f);}else{resizeImage(f,1100,done);}}
function famSubmit(){if(!famFileData){toast('Seleccioná un archivo primero.','warn');return;}const p=famStudent.pagos.find(x=>x.id===$('#famPagoSel').value);if(p){p.comprobanteFile=famFileData.file;p.comprobanteNombre=famFileData.nombre;p.comprobanteTipo=famFileData.tipo;p.comprobante='comprobante';p.metodo=p.metodo||'Comprobante familia';}toast('Comprobante enviado. Pendiente de validación.','success');famFileData=null;saveDB();render();}
...
function savePago(sid,pid){if(!perm('editar_pagos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===document.getElementById('mp_s').value);
...
function deletePago(sid,pid){if(!perm('eliminar_pagos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const p=s.pagos.find(x=>x.id===pid);openModal(confirmHTML('Eliminar cuota',`¿Eliminar la cuota <b>${p.mes}</b> de <b>${studentName(s)}</b> (${fmt(p.importe)})? Queda registrado en auditoría.`,`confirmDeletePago('${sid}','${pid}')`,'Eliminar'));}
function confirmDeletePago(sid,pid){if(!perm('eliminar_pagos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const i=s.pagos.findIndex(x=>x.id===pid);const p=s.pagos[i];logAudit('eliminar','Cuota · '+studentName(s),`Eliminada cuota ${p.mes} · ${fmt(p.importe)}`,'delete');s.pagos.splice(i,1);saveDB();closeModal();buildNav();render();toast('Cuota eliminada.','danger');}
function modalComprobante(sid,pid){const s=DB.students.find(x=>x.id===sid);const p=s.pagos.find(x=>x.id===pid);tmpComp=null;const puede=perm('editar_pagos');
  const prev=p.comprobanteFile?(p.comprobanteTipo==='pdf'?`<div class="doc-item" style="border:1px solid var(--line);border-radius:var(--r-sm);padding:10px"><div class="doc-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></svg></div><div class="info"><b>${esc(p.comprobanteNombre||'Comprobante.pdf')}</b><small>PDF adjunto</small></div><a class="btn btn-ghost btn-sm" href="${p.comprobanteFile}" target="_blank" rel="noopener">Abrir</a></div>`:`<img src="${p.comprobanteFile}" style="max-width:100%;border-radius:var(--r-sm);border:1px solid var(--line)">`):'<div class="empty" style="padding:24px">Sin comprobante adjunto.</div>';
  openModal(`<div class="modal-head"><h3>Comprobante · ${p.mes}</h3><button class="x" onclick="closeModal()">✕</button></div><div class="modal-body"><div style="margin-bottom:10px;color:var(--ink-soft)"><b>${studentName(s)}</b> · ${esc(p.concepto)} · ${fmt(p.importe)}</div><div id="compPrev" style="margin:12px 0">${prev}</div>${puede?`<input type="file" id="compFile" accept="image/*,.pdf" style="display:none" onchange="compPreview()"><div class="dropzone" onclick="$('#compFile').click()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M17 8l-5-5-5 5M12 3v12"/></svg><div><b>Subir comprobante</b></div><small>Imagen o PDF</small></div>`:''}</div><div class="modal-foot"><button class="btn btn-ghost" onclick="closeModal()">Cerrar</button>${puede?`<button class="btn btn-primary" onclick="saveComprobante('${sid}','${pid}')">Guardar comprobante</button>`:''}</div>`,true);}
function compPreview(){const f=$('#compFile').files[0];if(!f)return;const isPdf=/pdf$/i.test(f.type)||/\.pdf$/i.test(f.name);
  if(isPdf){const r=new FileReader();r.onload=e=>{tmpComp={file:e.target.result,nombre:f.name,tipo:'pdf'};$('#compPrev').innerHTML=`<div class="doc-item" style="border:1px solid var(--line);border-radius:var(--r-sm);padding:10px"><div class="doc-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></svg></div><div class="info"><b>${esc(f.name)}</b><small>PDF listo para guardar</small></div></div>`;};r.readAsDataURL(f);}
  else{resizeImage(f,1100,d=>{tmpComp={file:d,nombre:f.name,tipo:'image'};$('#compPrev').innerHTML=`<img src="${d}" style="max-width:100%;border-radius:var(--r-sm);border:1px solid var(--line)">`;});}}
function saveComprobante(sid,pid){if(!perm('editar_pagos')){toast('Sin permiso.','danger');return;}if(!tmpComp){toast('Subí un archivo primero.','warn');return;}const s=DB.students.find(x=>x.id===sid);const p=s.pagos.find(x=>x.id===pid);p.comprobanteFile=tmpComp.file;p.comprobanteNombre=tmpComp.nombre;p.comprobanteTipo=tmpComp.tipo;p.comprobante=p.comprobante||'adjunto';tmpComp=null;logAudit('editar','Comprobante · '+studentName(s),p.mes+' · comprobante adjuntado','edit');saveDB();closeModal();render();toast('Comprobante guardado.','success');}
...
function doGenerar(){if(!perm('editar_pagos')){toast('Sin permiso.','danger');return;}const mes=$('#gm_mes').value,com=+$('#gm_com').value,dia=$('#gm_dia').value;const venc=today.getFullYear()+'-'+String(today.getMonth()+1).padStart(2,'0')+'-'+String(dia).padStart(2,'0');let n=0;
...
function doImport(type){if(!perm('importar_pagos')&&!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}type=type||'cuotas';const f=$('#imp_file').files[0];if(!f){toast('Seleccioná un CSV.','warn');return;}const r=new FileReader();
...
function turnoCfg(){const o=(cfg().alumnoBuiltin||{}).turno||{};return {label:(o.label&&o.label.trim())||'Turno',opciones:(o.opciones&&o.opciones.length)?o.opciones:['Mañana','Tarde'],activo:o.activo===true};}
function setTurnoActivo(){if(!perm('editar_config'))return;ensureCfgArrays();cfg().alumnoBuiltin.turno=cfg().alumnoBuiltin.turno||{};cfg().alumnoBuiltin.turno.activo=!turnoCfg().activo;saveDB();render();}
function setTurnoLabel(v){if(!perm('editar_config'))return;ensureCfgArrays();cfg().alumnoBuiltin.turno=cfg().alumnoBuiltin.turno||{};cfg().alumnoBuiltin.turno.label=v.trim();saveDB();render();}
function setTurnoOpciones(v){if(!perm('editar_config'))return;ensureCfgArrays();cfg().alumnoBuiltin.turno=cfg().alumnoBuiltin.turno||{};cfg().alumnoBuiltin.turno.opciones=v.split(',').map(x=>x.trim()).filter(Boolean);saveDB();render();}
function cleanupExtras(){if(!DB||!DB.config)return;if(DB.config.extrasCleaned)return;const ca=cfg().camposAlumno;if(ca&&ca.length){const mat=/maternal/i.test(DB.config.nombre||'');const before=ca.length;cfg().camposAlumno=ca.filter(c=>{if(c.id==='cax_turno'||c.id==='cax_sala')return false;if(mat){const l=(c.label||'').trim().toLowerCase();if(l==='turno'||l==='sala')return false;}return true;});if(cfg().camposAlumno.length!==before){DB.config.extrasCleaned=true;try{saveDB();}catch(e){}return;}}DB.config.extrasCleaned=true;}
function migrateMaternal(){if(!DB||!DB.config||DB.config.setupMaternal)return;if(!/maternal/i.test(DB.config.nombre||''))return;ensureCfgArrays();const ab=cfg().alumnoBuiltin;ab.division=ab.division||{};ab.division.opciones=['A','B','1','2','3'];ab.turno=ab.turno||{};ab.turno.label='Turno';ab.turno.opciones=['Mañana','Tarde'];ab.turno.activo=true;
  const mover=(from,to)=>{DB.students.forEach(st=>{if(st.curso===from)st.curso=to;(st.inscripciones||[]).forEach(i=>{if(i.curso===from)i.curso=to;});});};
  const c4=DB.courses.find(x=>x.nombre==='Sala de 4');if(c4){c4.nombre='SALA';mover('Sala de 4','SALA');const i=CURSOS.indexOf('Sala de 4');if(i>=0)CURSOS[i]='SALA';}
  if(DB.courses.some(x=>x.nombre==='Sala de 5')){mover('Sala de 5','SALA');DB.courses=DB.courses.filter(x=>x.nombre!=='Sala de 5');const i=CURSOS.indexOf('Sala de 5');if(i>=0)CURSOS.splice(i,1);}
  if(!DB.courses.some(x=>x.nombre==='SALA')){DB.courses.unshift({id:'c'+Date.now(),nombre:'SALA',nivel:'Inicial',capacidad:20,cuota:0,turno:'Mañana',materias:[]});CURSOS.unshift('SALA');}
  DB.config.setupMaternal=true;try{saveDB();}catch(e){}}
...
function modalStudent(id){
  ...
  openModal(`<div class="modal-head"><h3>${isNew?'Nuevo':'Editar'} alumno</h3><button class="x" onclick="closeModal()">✕</button></div><div class="modal-body"><div class="form-grid">
     ...
     ${turnoCfg().activo?`<div class="field full"><label>${esc(turnoCfg().label)}</label><select id="ms_turno">${turnoCfg().opciones.map(o=>`<option ${s.turno===o?'selected':''}>${esc(o)}</option>`).join('')}</select></div>`:''}
...
function saveStudent(id){if(!perm('editar_alumnos')){toast('Sin permiso para editar alumnos.','danger');return;}
  ...
  const data={nombre:$('#ms_n').value,apellidos:$('#ms_a').value,curso,grupo:$('#ms_g').value,turno:(g('ms_turno')?g('ms_turno').value:(id?(DB.students.find(x=>x.id===id)||{}).turno:'')||''),contactos,
...
function panelFormAlumno(){const bi=ALUMNO_BUILTIN.map(([k])=>{const c=builtinCfg(k);return `<div class="alert-item"><div class="info" style="flex:1"><input value="${esc(c.label)}" onchange="setBuiltinLabel('${k}',this.value)" style="width:100%;max-width:240px;padding:7px 10px;border:1.5px solid var(--line);border-radius:8px;font-weight:600"><small>Campo del sistema</small></div><div class="switch ${c.activo?'on':''}" onclick="toggleBuiltin('${k}',this)"></div></div>`;}).join('');
  ...
  return `<div class="panel" style="margin-top:16px"><div class="panel-head"><h3>Formulario de alumno</h3><span class="sub">Activá, renombrá o agregá campos del alta de alumnos</span><div class="right"><button class="btn btn-primary btn-sm" onclick="modalCampoAlumno()">+ Nuevo campo</button></div></div><div class="panel-body">
     ...
     <div class="form-grid"><div class="field"><label>Nombre del campo</label><input value="${esc(divisionCfg().label)}" onchange="setDivisionLabel(this.value)" placeholder="Ej: División"></div><div class="field"><label>Opciones (separadas por coma)</label><input value="${esc(divisionCfg().opciones.join(', '))}" onchange="setDivisionOpciones(this.value)" placeholder="A, B, 1, 2, 3"></div></div>
     <div class="form-section">Turno (debajo de División)</div>
     <div class="alert-item"><div class="info" style="flex:1"><input value="${esc(turnoCfg().label)}" onchange="setTurnoLabel(this.value)" style="width:100%;max-width:240px;padding:7px 10px;border:1.5px solid var(--line);border-radius:8px;font-weight:600"><small>Mostrar selector de turno en el alta</small></div><div class="switch ${turnoCfg().activo?'on':''}" onclick="setTurnoActivo()"></div></div>
     <div class="field" style="max-width:420px"><label>Opciones (separadas por coma)</label><input value="${esc(turnoCfg().opciones.join(', '))}" onchange="setTurnoOpciones(this.value)" placeholder="Mañana, Tarde"></div>
...
function saveFoto(id){if(!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);if(tmpFoto)s.foto=tmpFoto;const auth=document.getElementById('foto_auth').classList.contains('on');s.autorizacionImagen={permitida:auth,fecha:auth?today.toISOString().slice(0,10):''};tmpFoto=null;logAudit('editar','Foto · '+studentName(s),'Foto / autorización actualizada','edit');saveDB();closeModal();render();toast('Foto guardada.','success');}
...
function saveDoc(id){if(!(perm('editar_documentos')||perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);const f=document.getElementById('md_file').files[0];const tipo=$('#md_tipo').value;s.documentos.push({id:'d'+Date.now(),tipo,nombre:f?f.name:tipo.replace(/ /g,'_')+'.pdf',fecha:$('#md_fecha').value,vencimiento:$('#md_venc').value});logAudit('crear','Documento · '+studentName(s),tipo+' agregado','create');saveDB();closeModal();render();toast('Documento agregado.','success');}
function deleteDoc(sid,did){if(!(perm('editar_documentos')||perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}openModal(confirmHTML('Eliminar documento','¿Eliminar este documento del legajo? Queda registrado en auditoría.',`confirmDeleteDoc('${sid}','${did}')`,'Eliminar'));}
function confirmDeleteDoc(sid,did){if(!(perm('editar_documentos')||perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const i=s.documentos.findIndex(d=>d.id===did);const d=s.documentos[i];logAudit('eliminar','Documento · '+studentName(s),d.tipo+' eliminado','delete');s.documentos.splice(i,1);saveDB();closeModal();render();toast('Documento eliminado.','danger');}
...
function doSend(){if(!perm('ver_comunicaciones')){toast('Sin permiso.','danger');return;}const dest=$('#sd_dest').value;const t=DB.templates.find(x=>x.id===$('#sd_tpl').value);if(!dest){toast('Seleccioná un destinatario.','warn');return;}
...
function saveTemplate(id){if(!perm('ver_comunicaciones')){toast('Sin permiso.','danger');return;}const data={nombre:$('#mt_n').value,categoria:$('#mt_c').value,asunto:$('#mt_a').value,cuerpo:$('#mt_b').value};if(!data.nombre){toast('Indicá el nombre.','warn');return;}
  ...
  saveDB();closeModal();render();}
function deleteTemplate(id){if(!(perm('ver_comunicaciones'))){toast('Sin permiso.','danger');return;}const t=DB.templates.find(x=>x.id===id);openModal(confirmHTML('Eliminar plantilla',`¿Eliminar la plantilla <b>${t.nombre}</b>?`,`confirmDeleteTemplate('${id}')`,'Eliminar'));}
function confirmDeleteTemplate(id){if(!(perm('ver_comunicaciones'))){toast('Sin permiso.','danger');return;}const t=DB.templates.find(x=>x.id===id);DB.templates=DB.templates.filter(x=>x.id!==id);logAudit('eliminar','Plantilla · '+t.nombre,'Plantilla eliminada','delete');saveDB();closeModal();render();toast('Plantilla eliminada.','danger');}
...
function doInscribir(sid){if(!(perm('ver_inscripciones'))){toast('Sin permiso.','danger');return;}inscribirAlumnoCurso(sid,$('#mi_curso').value,inscCiclo);saveDB();closeModal();render();}
...
function saveCurso(){if(!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}syncMat();const c=DB.courses.find(x=>x.id===tmpCourseId);if(!c)return;const prev=c.nombre;
...
function saveActividad(id){
  ...
  saveDB();closeModal();render();}
function deleteActividad(id){if(!(perm('ver_actividades'))){toast('Sin permiso.','danger');return;}const a=getAct(id);openModal(confirmHTML('Eliminar actividad',`¿Eliminar <b>${a.nombre}</b>? Se da de baja a ${actConfirmados(id).length+actEspera(id).length} alumno(s).`,`confirmDeleteActividad('${id}')`,'Eliminar'));}
function confirmDeleteActividad(id){if(!(perm('ver_actividades'))){toast('Sin permiso.','danger');return;}const a=getAct(id);DB.students.forEach(s=>{s.actividades=(s.actividades||[]).filter(e=>e.id!==id);});DB.activities=DB.activities.filter(x=>x.id!==id);logAudit('eliminar','Actividad · '+a.nombre,'Actividad eliminada','delete');saveDB();closeModal();render();toast('Actividad eliminada.','danger');}
...
async function saveUser(id){const btn=event&&event.currentTarget;if(btn&&btn.disabled)return;if(btn){btn.disabled=true;btn.textContent='Guardando...';}
  ...
  saveDB();
  // Sincronizar con el servidor (Auth + RLS): el usuario podrá entrar con su rol real.
  if(window.AuloraBackend&&window.AuloraBackend.enabled&&window.AuloraBackend.upsertProfile){
    try{const r=await window.AuloraBackend.upsertProfile(data.email,data.nombre,data.rol,data.activo);toast(r==='pendiente'?'Usuario guardado. Quedará activo cuando entre por primera vez.':'Usuario guardado y sincronizado.','success');}
...
function saveAutorizado(id){if(!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);const nombre=$('#au_n').value.trim();if(!nombre){toast('Indicá el nombre.','warn');return;}const dni=$('#au_d').value.trim();if(dni&&!/^\d{7,8}$/.test(dni)){toast('DNI inválido (7 u 8 dígitos).','warn');return;}s.autorizados=s.autorizados||[];s.autorizados.push({nombre,dni,relacion:$('#au_r').value||'—'});logAudit('crear','Autorizado · '+studentName(s),nombre+' habilitado para retirar','create');saveDB();closeModal();render();toast('Persona autorizada.','success');}
function delAutorizado(id,idx){if(!(perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);const a=s.autorizados[idx];s.autorizados.splice(idx,1);logAudit('eliminar','Autorizado · '+studentName(s),a.nombre+' dado de baja','delete');saveDB();render();toast('Autorizado eliminado.','danger');}
...
function doRetiro(id){if(!(perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);s.retiros=s.retiros||[];s.retiros.push({fecha:$('#rt_f').value,quien:$('#rt_q').value,registradoPor:CURRENT.nombre});logAudit('crear','Retiro · '+studentName(s),'Retirado por '+$('#rt_q').value,'info');saveDB();closeModal();render();toast('Retiro registrado.','success');}
...
function saveIncidencia(id){if(!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);const d=$('#in_d').value.trim();if(!d){toast('Describí la incidencia.','warn');return;}s.incidencias=s.incidencias||[];s.incidencias.push({id:'inc'+Date.now(),fecha:new Date().toISOString().slice(0,16).replace('T',' '),tipo:$('#in_t').value,gravedad:$('#in_g').value,descripcion:d,autor:CURRENT.nombre,estado:'Abierta'});logAudit('crear','Incidencia · '+studentName(s),$('#in_t').value+' ('+$('#in_g').value+')','create');saveDB();closeModal();render();toast('Incidencia registrada.','success');}
function toggleIncidencia(id,iid){if(!(perm('editar_alumnos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===id);const x=s.incidencias.find(z=>z.id===iid);x.estado=x.estado==='Abierta'?'Cerrada':'Abierta';logAudit('editar','Incidencia · '+studentName(s),'Estado: '+x.estado,'edit');saveDB();render();}
...
function doReinscribir(origen){if(!(perm('ver_inscripciones'))){toast('Sin permiso.','danger');return;}let n=0,eg=0;DB.students.forEach(s=>{const i=getIns(s,origen);if(!i||i.estado!=='confirmada')return;if(getIns(s,inscCiclo))return;const sig=nextCourse(i.curso);if(!sig){eg++;return;}const c=courseByName(sig);const cupo=cursoConfirmados(sig,inscCiclo).length;const estado=cupo<c.capacidad?'confirmada':'pendiente';s.inscripciones.push({ciclo:inscCiclo,curso:sig,estado,fecha:today.toISOString().slice(0,10)});n++;});
...
function saveNota(sid,materia,tri,val){if(!perm('editar_alumnos')){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);s.notas=s.notas||{};s.notas[materia]=s.notas[materia]||{t1:null,t2:null,t3:null};s.notas[materia][tri]=val===''?null:Math.max(1,Math.min(10,+val));saveDB();}
...
/* ====================== INFORMES A FAMILIAS ====================== */
let _infDraft=null;
function DEFAULT_INFORME_PLANTILLAS(){return [
 {id:'pi_inicial',nombre:'Informe — Nivel Inicial / Maternal',nivel:'Inicial',activo:true,escala:['Logrado','En proceso','Iniciando','No observado'],secciones:[
   {id:'s1',titulo:'Desarrollo socioemocional',items:[{id:'i1',texto:'Vínculo con pares y docentes'},{id:'i2',texto:'Expresión y regulación de emociones'},{id:'i3',texto:'Autonomía e iniciativa'},{id:'i4',texto:'Respeto de normas y límites'}]},
   {id:'s2',titulo:'Comunicación y lenguaje',items:[{id:'i5',texto:'Comprensión de consignas'},{id:'i6',texto:'Expresión verbal'},{id:'i7',texto:'Interés por cuentos y relatos'}]},
   {id:'s3',titulo:'Motricidad',items:[{id:'i8',texto:'Motricidad gruesa (correr, saltar, trepar)'},{id:'i9',texto:'Motricidad fina (encastres, trazos)'},{id:'i10',texto:'Coordinación y equilibrio'}]},
   {id:'s4',titulo:'Hábitos y cuidado',items:[{id:'i11',texto:'Alimentación'},{id:'i12',texto:'Higiene'},{id:'i13',texto:'Orden y cuidado de materiales'}]},
   {id:'s5',titulo:'Juego y cognición',items:[{id:'i14',texto:'Juego simbólico y con otros'},{id:'i15',texto:'Atención y concentración'},{id:'i16',texto:'Resolución de problemas'},{id:'i17',texto:'Creatividad y exploración'}]}
 ]},
 {id:'pi_primaria',nombre:'Informe — Primaria',nivel:'Primaria',activo:true,escala:['Muy bueno','Bueno','Satisfactorio','En proceso','Necesita apoyo'],secciones:[
   {id:'s1',titulo:'Áreas curriculares',items:[{id:'i1',texto:'Lengua'},{id:'i2',texto:'Matemática'},{id:'i3',texto:'Ciencias Naturales'},{id:'i4',texto:'Ciencias Sociales'}]},
   {id:'s2',titulo:'Aspectos actitudinales',items:[{id:'i5',texto:'Responsabilidad y hábitos de estudio'},{id:'i6',texto:'Participación en clase'},{id:'i7',texto:'Convivencia y respeto'},{id:'i8',texto:'Autonomía'},{id:'i9',texto:'Trabajo en equipo'}]}
 ]},
 {id:'pi_secundaria',nombre:'Informe — Secundaria',nivel:'Secundaria',activo:true,escala:['Muy bueno','Bueno','Satisfactorio','En proceso','Necesita apoyo'],secciones:[
   {id:'s1',titulo:'Desempeño académico',items:[{id:'i1',texto:'Comprensión de contenidos'},{id:'i2',texto:'Producción y entrega de trabajos'},{id:'i3',texto:'Desempeño en evaluaciones'}]},
   {id:'s2',titulo:'Aspectos actitudinales',items:[{id:'i4',texto:'Responsabilidad'},{id:'i5',texto:'Participación'},{id:'i6',texto:'Convivencia y respeto'},{id:'i7',texto:'Compromiso con el aprendizaje'}]}
 ]}
];}
function informePlantillas(){if(!DB.informePlantillas||!DB.informePlantillas.length){DB.informePlantillas=DEFAULT_INFORME_PLANTILLAS();try{saveDB();}catch(e){}}return DB.informePlantillas;}
function plantillaById(id){return informePlantillas().find(p=>p.id===id);}
function infValColor(pl,val){const i=pl.escala.indexOf(val);if(i<0)return'b-grey';if(pl.escala.length<=1||i===0)return'b-success';if(i===pl.escala.length-1)return'b-danger';if(i<=Math.floor((pl.escala.length-1)/2))return'b-info';return'b-warn';}
const SEG_AREAS=['General','Conducta / convivencia','Aprendizaje','Socioemocional','Salud','Comunicación con familia'];
function canInf(){return perm('gestionar_informes')||perm('editar_alumnos');}
function tabInformes(s){const can=canInf();const seg=(s.seguimiento||[]).slice().reverse();const infs=(s.informes||[]).slice().reverse();
 return `<div class="detail-grid">
   <div class="panel"><div class="panel-head"><h3>Seguimiento del docente</h3><span class="sub">Notas que se acumulan en el tiempo</span></div><div class="panel-body">
     ${can?`<div class="form-grid"><div class="field"><label>Área</label><select id="seg_area">${SEG_AREAS.map(a=>`<option>${a}</option>`).join('')}</select></div><div class="field"><label>Visible para la familia</label><select id="seg_vis"><option value="0">No (interno)</option><option value="1">Sí</option></select></div></div><div class="field"><textarea id="seg_txt" placeholder="Observación del día (logro, dificultad, hecho a destacar...)"></textarea></div><button class="btn btn-primary btn-sm" onclick="addSeguimiento('${s.id}')">Agregar nota</button><div style="height:14px"></div>`:''}
     ${seg.length?seg.map(o=>`<div class="timeline-item"><div class="dot"></div><div class="body">${esc(o.texto)}<div class="who">${esc(o.area||'General')} · ${o.autor} · ${o.fecha} ${o.visibleFamilia?'<span class="badge b-success nodot">Visible familia</span>':'<span class="badge b-grey nodot">Interno</span>'}</div></div>${can?`<div class="row-actions"><button class="icon-btn" title="${o.visibleFamilia?'Ocultar a familia':'Mostrar a familia'}" onclick="toggleSegVis('${s.id}','${o.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button><button class="icon-btn danger" onclick="delSeguimiento('${s.id}','${o.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button></div>`:''}</div>`).join(''):'<div class="empty">Sin notas de seguimiento.</div>'}
   </div></div>
   <div class="panel"><div class="panel-head"><h3>Informes a familias</h3>${can?`<div class="right"><button class="btn btn-primary btn-sm" onclick="modalInforme('${s.id}')">+ Nuevo informe</button></div>`:''}</div><div class="panel-body">
     ${infs.length?infs.map(inf=>{const pl=plantillaById(inf.plantillaId);return `<div class="alert-item"><div class="doc-ic" style="background:var(--blue-soft);color:var(--blue)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></svg></div><div class="info"><b>${esc(inf.periodoTipo)} · ${esc(inf.periodo)}</b><small>${pl?esc(pl.nombre):esc(inf.plantillaNombre||'Plantilla eliminada')} · ${inf.autor} · ${inf.fecha}</small></div><span class="badge ${inf.estado==='publicado'?'b-success':'b-warn'} nodot">${inf.estado==='publicado'?'Publicado':'Borrador'}</span><div class="row-actions"><button class="icon-btn" title="PDF" onclick="informePDF('${s.id}','${inf.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6M9 13h6M9 17h4"/></svg></button>${can?`<button class="icon-btn" title="${inf.estado==='publicado'?'Pasar a borrador':'Publicar a familia'}" onclick="togglePublicarInforme('${s.id}','${inf.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4z"/></svg></button><button class="icon-btn" title="Editar" onclick="modalInforme('${s.id}','${inf.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 013 3L12 15l-4 1 1-4z"/></svg></button><button class="icon-btn danger" onclick="delInforme('${s.id}','${inf.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>`:''}</div></div>`;}).join(''):'<div class="empty">Sin informes. Creá el primero para la familia.</div>'}
   </div></div>
 </div>`;}
function addSeguimiento(sid){if(!canInf()){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const t=$('#seg_txt').value.trim();if(!t){toast('Escribí la observación.','warn');return;}s.seguimiento=s.seguimiento||[];s.seguimiento.push({id:'sg'+Date.now(),fecha:new Date().toISOString().slice(0,16).replace('T',' '),autor:CURRENT.nombre,area:$('#seg_area').value,texto:t,visibleFamilia:$('#seg_vis').value==='1'});logAudit('crear','Seguimiento · '+studentName(s),'Nota de seguimiento','create');saveDB();render();toast('Nota agregada.','success');}
function toggleSegVis(sid,id){if(!canInf())return;const s=DB.students.find(x=>x.id===sid);const o=(s.seguimiento||[]).find(x=>x.id===id);if(!o)return;o.visibleFamilia=!o.visibleFamilia;saveDB();render();}
function delSeguimiento(sid,id){if(!canInf())return;const s=DB.students.find(x=>x.id===sid);s.seguimiento=(s.seguimiento||[]).filter(x=>x.id!==id);saveDB();render();toast('Nota eliminada.','danger');}
function infSwitch(sid,infId,plId){_infDraft={periodoTipo:$('#inf_tipo').value,periodo:$('#inf_per').value,fortalezas:$('#inf_fort').value,aMejorar:$('#inf_mej').value,recomendaciones:$('#inf_rec').value,comentario:$('#inf_com').value,incluirSeguimiento:$('#inf_seg').classList.contains('on'),plId};modalInforme(sid,infId);}
function modalInforme(sid,infId){if(!canInf()){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const pls=informePlantillas().filter(p=>p.activo!==false);if(!pls.length){toast('No hay plantillas de informe activas.','warn');return;}
 const inf=infId?(s.informes||[]).find(x=>x.id===infId):null;const d=_infDraft||{};
 const plId=d.plId||(inf?inf.plantillaId:pls[0].id);const pl=plantillaById(plId)||pls[0];
 const vals=_infDraft?{}:(inf?inf.valores||{}:{});
 const tipo=d.periodoTipo!==undefined?d.periodoTipo:(inf?inf.periodoTipo:'Mensual');
 const periodo=d.periodo!==undefined?d.periodo:(inf?inf.periodo:'');
 const fort=d.fortalezas!==undefined?d.fortalezas:(inf?inf.fortalezas:'');
 const mej=d.aMejorar!==undefined?d.aMejorar:(inf?inf.aMejorar:'');
 const recm=d.recomendaciones!==undefined?d.recomendaciones:(inf?inf.recomendaciones:'');
 const com=d.comentario!==undefined?d.comentario:(inf?inf.comentario:'');
 const incSeg=_infDraft?d.incluirSeguimiento:(inf?inf.incluirSeguimiento!==false:true);
 const segVis=(s.seguimiento||[]).filter(o=>o.visibleFamilia).length;_infDraft=null;
 openModal(`<div class="modal-head"><h3>${infId?'Editar informe':'Nuevo informe'} · ${studentName(s)}</h3><button class="x" onclick="closeModal()">✕</button></div><div class="modal-body">
   <div class="form-grid"><div class="field"><label>Plantilla</label><select id="inf_pl" onchange="infSwitch('${sid}','${infId||''}',this.value)">${pls.map(p=>`<option value="${p.id}" ${p.id===plId?'selected':''}>${esc(p.nombre)}</option>`).join('')}</select></div>
     <div class="field"><label>Tipo de período</label><select id="inf_tipo">${['Mensual','Trimestral','Anual'].map(x=>`<option ${tipo===x?'selected':''}>${x}</option>`).join('')}</select></div>
     <div class="field"><label>Período</label><input id="inf_per" value="${esc(periodo)}" placeholder="Ej: Junio 2026 / 1.º Trimestre"></div></div>
   ${pl.secciones.map(sec=>`<div class="form-section">${esc(sec.titulo)}</div>${sec.items.map(it=>`<div class="field" style="margin-bottom:8px"><label>${esc(it.texto)}</label><select data-it="${it.id}" class="inf_item"><option value="">— No evaluado</option>${pl.escala.map(e=>`<option ${vals[it.id]===e?'selected':''}>${esc(e)}</option>`).join('')}</select></div>`).join('')}`).join('')}
   <div class="form-section">Comentarios para la familia</div>
   <div class="field"><label>Fortalezas</label><textarea id="inf_fort">${esc(fort)}</textarea></div>
   <div class="field"><label>Aspectos a mejorar</label><textarea id="inf_mej">${esc(mej)}</textarea></div>
   <div class="field"><label>Recomendaciones para casa</label><textarea id="inf_rec">${esc(recm)}</textarea></div>
   <div class="field"><label>Comentario general</label><textarea id="inf_com">${esc(com)}</textarea></div>
   <div class="full check"><div class="switch ${incSeg?'on':''}" id="inf_seg" onclick="this.classList.toggle('on')"></div><span>Incluir las notas de seguimiento visibles (${segVis}) en el PDF</span></div>
   </div><div class="modal-foot"><button class="btn btn-ghost" onclick="closeModal()">Cancelar</button><button class="btn btn-ghost" onclick="saveInforme('${sid}','${infId||''}',false)">Guardar borrador</button><button class="btn btn-primary" onclick="saveInforme('${sid}','${infId||''}',true)">Guardar y publicar</button></div>`,true);}
function saveInforme(sid,infId,publicar){if(!canInf()){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const plId=$('#inf_pl').value;const pl=plantillaById(plId);const periodo=$('#inf_per').value.trim();if(!periodo){toast('Indicá el período.','warn');return;}const valores={};document.querySelectorAll('.inf_item').forEach(el=>{if(el.value)valores[el.dataset.it]=el.value;});
 const base={plantillaId:plId,plantillaNombre:pl?pl.nombre:'',periodoTipo:$('#inf_tipo').value,periodo,valores,fortalezas:$('#inf_fort').value.trim(),aMejorar:$('#inf_mej').value.trim(),recomendaciones:$('#inf_rec').value.trim(),comentario:$('#inf_com').value.trim(),incluirSeguimiento:$('#inf_seg').classList.contains('on'),estado:publicar?'publicado':'borrador'};
 s.informes=s.informes||[];
 if(infId){const inf=s.informes.find(x=>x.id===infId);Object.assign(inf,base);if(publicar&&!inf.fechaPublicado)inf.fechaPublicado=today.toISOString().slice(0,10);}
 else{s.informes.push({id:'inf'+Date.now(),fecha:new Date().toISOString().slice(0,16).replace('T',' '),autor:CURRENT.nombre,...base,fechaPublicado:publicar?today.toISOString().slice(0,10):''});}
 if(publicar)notificarInforme(s,periodo);
 logAudit(infId?'editar':'crear','Informe · '+studentName(s),base.periodoTipo+' '+periodo+(publicar?' (publicado)':' (borrador)'),infId?'edit':'create');saveDB();closeModal();render();toast(publicar?'Informe publicado a la familia.':'Borrador guardado.','success');}
function notificarInforme(s,periodo){DB.mensajes=DB.mensajes||[];DB.mensajes.push({id:'m'+Date.now()+Math.random().toString(16).slice(2,5),studentId:s.id,asunto:'Nuevo informe de seguimiento — '+periodo,fecha:new Date().toISOString().slice(0,16).replace('T',' '),leido:false});}
function togglePublicarInforme(sid,id){if(!canInf())return;const s=DB.students.find(x=>x.id===sid);const inf=(s.informes||[]).find(x=>x.id===id);if(!inf)return;if(inf.estado==='publicado'){inf.estado='borrador';}else{inf.estado='publicado';if(!inf.fechaPublicado)inf.fechaPublicado=today.toISOString().slice(0,10);notificarInforme(s,inf.periodo);}logAudit('editar','Informe · '+studentName(s),'Estado: '+inf.estado,'edit');saveDB();render();toast(inf.estado==='publicado'?'Publicado a la familia.':'Pasado a borrador.','success');}
function delInforme(sid,id){if(!canInf())return;openModal(confirmHTML('Eliminar informe','¿Eliminar este informe? No se puede deshacer.',`confirmDelInforme('${sid}','${id}')`,'Eliminar'));}
function confirmDelInforme(sid,id){if(!canInf())return;const s=DB.students.find(x=>x.id===sid);s.informes=(s.informes||[]).filter(x=>x.id!==id);logAudit('eliminar','Informe · '+studentName(s),'Informe eliminado','delete');saveDB();closeModal();render();toast('Informe eliminado.','danger');}
function informePDF(sid,id){const s=DB.students.find(x=>x.id===sid);const inf=(s.informes||[]).find(x=>x.id===id);if(!inf)return;if(!window.jspdf){toast('No se pudo cargar el PDF.','danger');return;}const {jsPDF}=window.jspdf;const doc=new jsPDF();const pl=plantillaById(inf.plantillaId);
 doc.setFontSize(16);doc.setFont(undefined,'bold');doc.text(cfg().centroNombre,14,18);doc.setFontSize(10);doc.setFont(undefined,'normal');doc.text(cfg().direccion+' · '+cfg().provincia,14,24);doc.setDrawColor(200);doc.line(14,28,196,28);
 doc.setFontSize(13);doc.setFont(undefined,'bold');doc.text('Informe de seguimiento — '+inf.periodoTipo+' '+inf.periodo,14,38);
 doc.setFontSize(11);doc.setFont(undefined,'normal');doc.text('Alumno/a: '+studentName(s),14,47);doc.text('Curso: '+s.curso+(s.grupo?' · '+s.grupo:'')+(s.turno?' · Turno '+s.turno:''),14,53);
 try{const ar=asistResumen(s);if(ar)doc.text('Asistencia del ciclo: '+ar.pct+'%',14,59);}catch(e){}
 let y=70;
 (pl?pl.secciones:[]).forEach(sec=>{if(y>270){doc.addPage();y=20;}doc.setFont(undefined,'bold');doc.setFontSize(11);doc.text(String(sec.titulo),14,y);y+=7;doc.setFont(undefined,'normal');doc.setFontSize(10);sec.items.forEach(it=>{if(y>278){doc.addPage();y=20;}const v=inf.valores[it.id]||'—';doc.text(String(it.texto).slice(0,72),16,y);doc.text(String(v),150,y);y+=6;});y+=3;});
 const blk=(t,v)=>{if(!v)return;if(y>258){doc.addPage();y=20;}doc.setFont(undefined,'bold');doc.setFontSize(11);doc.text(t,14,y);y+=6;doc.setFont(undefined,'normal');doc.setFontSize(10);const ls=doc.splitTextToSize(v,180);doc.text(ls,14,y);y+=ls.length*5+4;};
 blk('Fortalezas',inf.fortalezas);blk('Aspectos a mejorar',inf.aMejorar);blk('Recomendaciones para casa',inf.recomendaciones);blk('Comentario general',inf.comentario);
 if(inf.incluirSeguimiento){const notas=(s.seguimiento||[]).filter(o=>o.visibleFamilia);if(notas.length){if(y>250){doc.addPage();y=20;}doc.setFont(undefined,'bold');doc.setFontSize(11);doc.text('Notas de seguimiento',14,y);y+=6;doc.setFont(undefined,'normal');doc.setFontSize(9);notas.forEach(o=>{const ls=doc.splitTextToSize('• ['+o.fecha.slice(0,10)+'] '+o.texto,180);if(y+ls.length*4>285){doc.addPage();y=20;}doc.text(ls,14,y);y+=ls.length*4+2;});}}
 if(y>255){doc.addPage();y=20;}y+=12;doc.line(120,y,190,y);doc.setFontSize(10);doc.setTextColor(0);doc.text('Firma del docente',138,y+6);
 doc.setFontSize(9);doc.setTextColor(120);doc.text('Generado por Aulora · '+today.toISOString().slice(0,10)+(inf.autor?' · '+inf.autor:''),14,292);
 doc.save('informe_'+s.apellidos+'_'+inf.periodo.replace(/[^a-zA-Z0-9]/g,'_')+'.pdf');logAudit('info','Informe · '+studentName(s),'PDF generado','info');saveDB();toast('Informe descargado.','success');}
function portalInformesHTML(s){const infs=(s.informes||[]).filter(i=>i.estado==='publicado').slice().reverse();if(!infs.length)return '';
 return `<div class="panel" style="margin-top:16px"><div class="panel-head"><h3>📋 Informes de seguimiento</h3><span class="sub">${infs.length} publicado(s)</span></div><div class="panel-body">${infs.map(inf=>{const pl=plantillaById(inf.plantillaId);return `<div style="border:1px solid var(--line);border-radius:12px;padding:14px;margin-bottom:12px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><b style="flex:1">${esc(inf.periodoTipo)} · ${esc(inf.periodo)}</b><button class="btn btn-ghost btn-sm" onclick="informePDF('${s.id}','${inf.id}')">📄 PDF</button></div>${pl?pl.secciones.map(sec=>{const its=sec.items.filter(it=>inf.valores[it.id]);if(!its.length)return'';return `<div style="margin-bottom:8px"><div style="font-size:12px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:4px">${esc(sec.titulo)}</div>${its.map(it=>`<div class="info-row"><span class="k">${esc(it.texto)}</span><span class="v"><span class="badge ${infValColor(pl,inf.valores[it.id])} nodot">${esc(inf.valores[it.id])}</span></span></div>`).join('')}</div>`;}).join(''):''}${inf.fortalezas?`<div style="margin-top:8px"><b style="font-size:12.5px">Fortalezas:</b> <span style="font-size:13px">${esc(inf.fortalezas)}</span></div>`:''}${inf.aMejorar?`<div><b style="font-size:12.5px">A mejorar:</b> <span style="font-size:13px">${esc(inf.aMejorar)}</span></div>`:''}${inf.recomendaciones?`<div><b style="font-size:12.5px">Para casa:</b> <span style="font-size:13px">${esc(inf.recomendaciones)}</span></div>`:''}${inf.comentario?`<div style="margin-top:6px;font-size:13px;font-style:italic">${esc(inf.comentario)}</div>`:''}<div style="font-size:11.5px;color:var(--muted);margin-top:8px">${inf.autor} · ${inf.fechaPublicado||inf.fecha}</div></div>`;}).join('')}</div></div>`;}
function panelInformes(){if(!perm('editar_config'))return '';const pls=informePlantillas();
 return `<div class="panel" style="margin-top:16px"><div class="panel-head"><h3>Plantillas de informe</h3><span class="sub">Lo que el docente evalúa en los informes a familias</span><div class="right"><button class="btn btn-primary btn-sm" onclick="modalPlantillaInforme()">+ Nueva plantilla</button></div></div><div class="panel-body">
   ${pls.map(p=>`<div class="alert-item"><div class="info" style="flex:1"><b>${esc(p.nombre)}</b><small>${esc(p.nivel||'')} · ${p.secciones.length} secciones · escala: ${esc(p.escala.join(', '))}</small></div><div class="switch ${p.activo!==false?'on':''}" onclick="togglePlantilla('${p.id}')"></div><button class="btn btn-ghost btn-sm" onclick="modalPlantillaInforme('${p.id}')">Editar</button><button class="icon-btn danger" onclick="delPlantillaInforme('${p.id}')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button></div>`).join('')}
 </div></div>`;}
function plToText(p){return p.secciones.map(sec=>sec.titulo+': '+sec.items.map(i=>i.texto).join('; ')).join('\n');}
function modalPlantillaInforme(plId){if(!perm('editar_config')){toast('Sin permiso.','danger');return;}const p=plId?plantillaById(plId):{nombre:'',nivel:'',escala:['Logrado','En proceso','Iniciando'],secciones:[]};
 openModal(`<div class="modal-head"><h3>${plId?'Editar plantilla':'Nueva plantilla'}</h3><button class="x" onclick="closeModal()">✕</button></div><div class="modal-body">
   <div class="form-grid"><div class="field"><label>Nombre</label><input id="pli_n" value="${esc(p.nombre)}"></div><div class="field"><label>Nivel</label><input id="pli_niv" value="${esc(p.nivel||'')}" placeholder="Inicial / Primaria / Secundaria"></div></div>
   <div class="field"><label>Escala de valoración (coma, de mejor a peor)</label><input id="pli_esc" value="${esc(p.escala.join(', '))}" placeholder="Logrado, En proceso, Iniciando"></div>
   <div class="field"><label>Secciones e ítems</label><textarea id="pli_sec" style="min-height:180px" placeholder="Título de la sección: ítem 1; ítem 2; ítem 3">${esc(plToText(p))}</textarea><small style="color:var(--muted)">Una línea por sección. Formato: <b>Título: ítem; ítem; ítem</b></small></div>
   </div><div class="modal-foot"><button class="btn btn-ghost" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="savePlantillaInforme('${plId||''}')">Guardar plantilla</button></div>`,true);}
function savePlantillaInforme(plId){if(!perm('editar_config')){toast('Sin permiso.','danger');return;}const nombre=$('#pli_n').value.trim();if(!nombre){toast('Indicá el nombre.','warn');return;}const escala=$('#pli_esc').value.split(',').map(x=>x.trim()).filter(Boolean);if(!escala.length){toast('Definí al menos una opción de escala.','warn');return;}
 const lines=$('#pli_sec').value.split('\n').map(l=>l.trim()).filter(Boolean);let sc=0,ic=0;
 const secciones=lines.map(l=>{const idx=l.indexOf(':');const titulo=(idx>=0?l.slice(0,idx):l).trim();const rest=idx>=0?l.slice(idx+1):'';const items=rest.split(';').map(x=>x.trim()).filter(Boolean).map(t=>({id:'i'+(++ic),texto:t}));return {id:'s'+(++sc),titulo,items};}).filter(x=>x.titulo);
 informePlantillas();
 if(plId){const p=plantillaById(plId);p.nombre=nombre;p.nivel=$('#pli_niv').value.trim();p.escala=escala;p.secciones=secciones;}
 else{DB.informePlantillas.push({id:'pi'+Date.now(),nombre,nivel:$('#pli_niv').value.trim(),escala,secciones,activo:true});}
 logAudit(plId?'editar':'crear','Plantilla informe · '+nombre,'Plantilla de informe','edit');saveDB();closeModal();render();toast('Plantilla guardada.','success');}
function togglePlantilla(plId){if(!perm('editar_config'))return;const p=plantillaById(plId);if(!p)return;p.activo=p.activo===false?true:false;saveDB();render();}
function delPlantillaInforme(plId){if(!perm('editar_config'))return;openModal(confirmHTML('Eliminar plantilla','¿Eliminar esta plantilla? Los informes ya creados se conservan.',`confirmDelPlantilla('${plId}')`,'Eliminar'));}
function confirmDelPlantilla(plId){if(!perm('editar_config'))return;DB.informePlantillas=informePlantillas().filter(p=>p.id!==plId);logAudit('eliminar','Plantilla informe','Plantilla eliminada','delete');saveDB();closeModal();render();toast('Plantilla eliminada.','danger');}
...
function marcarPagado(sid,pid){if(!(perm('editar_pagos'))){toast('Sin permiso.','danger');return;}const s=DB.students.find(x=>x.id===sid);const p=s.pagos.find(x=>x.id===pid);p.estado='pagado';p.fechaPago=today.toISOString().slice(0,10);p.metodo='Mercado Pago';p.comprobante='mp';logAudit('editar','Cuota · '+studentName(s),'Pago acreditado por Mercado Pago (demo)','edit');saveDB();closeModal();buildNav();render();toast('Pago acreditado.','success');}
...
/* ====================== PAÍS / CALENDARIO ====================== */
function setPais(p){if(!(perm('editar_config'))){toast('Sin permiso.','danger');return;}const c=cfg();c.pais=p;const d=PAIS_DEFAULTS[p]||PAIS_DEFAULTS.AR;c.locale=d.locale;c.moneda=d.moneda;logAudit('editar','Configuración del colegio','País: '+(p==='ES'?'España':'Argentina'),'edit');saveDB();render();toast('País: '+(p==='ES'?'España':'Argentina')+'. Aplicá el calendario escolar si corresponde.','success');}
function aplicarCalendarioPais(){if(!(perm('editar_config'))){toast('Sin permiso.','danger');return;}const sel=document.getElementById('cf_cal');const p=sel?sel.value:(cfg().pais||'AR');const userEv=(DB.eventos||[]).filter(e=>!e.preset);DB.eventos=calendarFor(p).concat(userEv);logAudit('editar','Calendario escolar','Preset '+(p==='ES'?'España':'Argentina')+' aplicado','edit');saveDB();render();toast('Calendario de '+(p==='ES'?'España':'Argentina')+' aplicado.','success');}
...
function saveContabMov(){if(!perm('editar_contabilidad')){toast('Sin permiso.','danger');return;}const cat=$('#cm_cat').value.trim();const monto=+$('#cm_m').value;if(!cat||!monto){toast('Completá categoría y monto.','warn');return;}DB.contab=DB.contab||{movimientos:[]};DB.contab.movimientos.push({id:'cm'+Date.now(),fecha:$('#cm_f').value,tipo:$('#cm_t').value,cat,detalle:$('#cm_det').value,monto});logAudit('crear','Contabilidad',cat+' · '+fmt(monto),'create');saveDB();closeModal();render();toast('Movimiento registrado.','success');}
function delContabMov(id){if(!perm('editar_contabilidad')){toast('Sin permiso.','danger');return;}DB.contab.movimientos=DB.contab.movimientos.filter(x=>x.id!==id);logAudit('eliminar','Contabilidad','Movimiento eliminado','delete');saveDB();render();toast('Movimiento eliminado.','danger');}
...
function saveAfaCuota(){if(!perm('editar_afa')){toast('Sin permiso.','danger');return;}DB.afa=DB.afa||{cuota:0,socios:[],movimientos:[]};DB.afa.cuota=+$('#afa_c').value||0;logAudit('editar','AFA / AMPA','Cuota actualizada','edit');saveDB();closeModal();render();toast('Cuota AFA actualizada.','success');}
...
function saveAfaMov(){if(!perm('editar_afa')){toast('Sin permiso.','danger');return;}const cat=$('#am_cat').value.trim();const monto=+$('#am_m').value;if(!cat||!monto){toast('Completá concepto y monto.','warn');return;}DB.afa=DB.afa||{cuota:0,socios:[],movimientos:[]};DB.afa.movimientos.push({id:'am'+Date.now(),fecha:$('#am_f').value,tipo:$('#am_t').value,cat,detalle:$('#am_det').value,monto});logAudit('crear','AFA / AMPA',cat+' · '+fmt(monto),'create');saveDB();closeModal();render();toast('Movimiento registrado.','success');}
function delAfaMov(id){if(!perm('editar_afa')){toast('Sin permiso.','danger');return;}DB.afa.movimientos=DB.afa.movimientos.filter(x=>x.id!==id);logAudit('eliminar','AFA / AMPA','Movimiento eliminado','delete');saveDB();render();toast('Movimiento eliminado.','danger');}
