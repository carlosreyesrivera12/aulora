...
if (PLACEHOLDER) {
  ...
  window.AuloraBackend = {
    ...
    myProfile: async () => { const { data: { user } } = await sb.auth.getUser(); if (!user) return null; const { data } = await sb.from('perfiles').select('rol,nombre,activo').eq('id', user.id).maybeSingle(); return data || null; },
